...
<cfoutput>
	...
	<cfset orderIDValue = variables.context.get("orderIDValue")>
	...
	<cfset pageAction = "EDITORDER">
	
	 <h1> <img src="images/pizza_icon.png" /> Edit Order </h1>
	...
	<!--- display a Pizza to edit --->
	
	<cfset pizzaIDValue = "">
	<cfif helperFuncs.isValidOrderID(orderIDValue)>
		<cfset cartStruct = cart.read(orderIDValue)>
		<cfif StructKeyExists(cartStruct, "PIZZAID")>
			<cfset pizzaIDValue = cartStruct.PIZZAID>
		</cfif>
	</cfif>
	...
	<cfif helperFuncs.isValidPizzaID(pizzaIDValue)>
		...
		<input id="Quantity" type="text" name="Quantity" value="#cartStruct.QUANTITY#" size="2" readonly="readonly"/> 
		...
		<table class="orderTable">
					...
					<td>
						...
						<select id="#topID#" name="#topID#" onchange="CalculateOrderTotal();">
							...
							<cfloop from="0" to="#val(topLimit)#" index="i">
								<option value="#i#" <cfif cartTopIDVal EQ i>selected="selected"</cfif>>#i#</option>
							</cfloop>
						</select>
						</td>
					</tr>
			...
			<p>
				<label for="price">Total Price: $</label> <input id="price" type="text" name="price" value="#val(cartStruct.PRICE)#" readonly="readonly"/>
